...
import { canonicalMastersContentRoute, h5LegacyRouteForMastersContent } from '../../../shared/masters/shareLinks';
...
export default function Prototype(){
 ...
 return <State.Provider value={{state,dispatch,warning}}><AppAccess.Provider value={(route,action)=>{setGate({route,action});}}><FlowStack initial={routes(initialRoute)}/><BottomSheet open={gate!==null} onOpenChange={open=>{if(!open)setGate(null);}} title={gate?.action==='我的关注'?'在 App 中查看关注':gate?.action==='我的收藏'?'在 App 中查看收藏':gate?.action==='关注'?'在 App 中关注这位法师或名家':gate?.action==='收藏'?'在 App 中收藏这份内容':'在 BuddhaChat 中继续'} description={gate?.action?.includes('关注')?'关注功能由 App 承接，网页可继续浏览各位主页。':gate?.action?.includes('收藏')?'收藏功能由 App 承接，网页可继续浏览公开内容。':'网页可继续阅读与播放公开内容。'}><p className="notice">此栏目的 App 内容页尚未上线。现在可以继续浏览网页，或打开 App 首页。</p><a className="primary wide" href={appHomeUrl}>打开 App 首页</a><a className="secondary wide" href={downloadUrl} target="_blank" rel="noopener noreferrer">尚未安装？前往下载</a><button className="text-button" onClick={()=>setGate(null)}>继续在网页浏览 <ArrowRightIcon/></button></BottomSheet></AppAccess.Provider></State.Provider>;
}
