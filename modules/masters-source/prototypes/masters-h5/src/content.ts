...
export function itemTitle(id:string) { return (id==='yh-dayi-001'?'答疑解惑 · 第一期':undefined) || shengyenCatalog.entries.find(b=>'sy-'+b.sourceId===id)?.title || readingContent.chapters.find(c=>c.id===id)?.title || nanItems.find(n=>n.id===id)?.title || books.find(b=>b.id===id)?.title || chapters.find(c=>c.id===id)?.title_display || videos.find(v=>v.id===id)?.title || id; }
export const clock = (seconds:number) => `${Math.floor(seconds/60)}:${String(Math.floor(seconds%60)).padStart(2,'0')}`;

export function itemOwner(id:string){return readingContent.chapters.find(c=>c.id===id)?.person_id || nanItems.find(n=>n.id===id)?.person_id || (id.startsWith('xy-')?'hsing-yun':id.startsWith('sy-')?'sheng-yen':undefined);}
