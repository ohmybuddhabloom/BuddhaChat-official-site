...
function changedFiles() {
  try {
    return execFileSync("git", ["diff", "--name-only", "--diff-filter=ACMRTUXB", `${previousSha}...HEAD`], {
      ...
      .split("\n")
      .map((line) => line.trim().replace(/^legal-site\//, ""))
      .filter(Boolean);
  } catch {
...
if (shouldBuild) {
  console.log("Relevant legal site files changed; building.");
  process.exit(1);
}

console.log("No relevant legal site files changed; skipping Vercel build.");
process.exit(0);
