...
2. Merge that exact candidate head into `staging`; the push automatically deploys the exact final
   `staging` SHA to the designated Standard Vercel Staging environment.
...
direct Vercel Production commands, when Git auto-deployment is enabled outside `staging`, when
`staging` auto-deployment is disabled, or when this policy is removed from the operating contract. This static guard
