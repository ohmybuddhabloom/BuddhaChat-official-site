...
function changedFiles() {
  try {
    return execFileSync("git", ["diff", "--name-only", "--diff-filter=ACMRTUXB", `${previousSha}...HEAD`], {
      ...
      .split("\n")
      .map((line) => line.trim().replace(/^master-router\//, ""))
      .filter(Boolean);
  } catch {
...
if (shouldBuild) {
  console.log("Relevant master router files changed; building.");
  process.exit(1);
}

console.log("No relevant master router files changed; skipping Vercel build.");
process.exit(0);
