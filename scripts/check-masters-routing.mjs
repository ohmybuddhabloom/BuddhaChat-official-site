...
assert.deepEqual(config.git.deploymentEnabled, { staging: true, '*': false, '**': false });
assert.ok(config.rewrites.some(route => route.source === '/download' && route.destination === '/api/page-entry?page=download'));
assert.ok(config.rewrites.some(route => route.source === '/download/android/latest.apk' && route.destination === '/api/android-apk'));
